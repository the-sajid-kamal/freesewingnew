...
they will be used primarily through the freesewing.eu website,
...
- If the `extraTopButton` option is set to `true`, then the
  `elasticatedCuff` option should be assigned to the `style` menu.
- If the `extraTopButton` option is set to `false`, setting the `menu`
  attribute to `false`, then the `elasticatedCuff` option should not
  appear in any menu.
...
The second parameter should be the return value of
[utils.mergeoptions()](/reference/api/utils/mergeoptions), which provides an
object with all option values populated.
This is required when the result depends on the value of another option.
